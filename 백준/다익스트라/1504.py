...
def dijkstra(start, end):
    ...
    print(f"\n[다익스트라 시작] {start} → {end}")
    print(f"초기 거리 배열: {route[1:]}")
    
    while q:
        print(f"\n현재 힙 상태: {q}")
        cnt_w, cnt_node = heapq.heappop(q)
        print(f"힙에서 꺼낸 노드: {cnt_node} (현재 거리: {cnt_w})")
        print(f"현재 거리 배열: {route[1:]}")
        
        if route[cnt_node] < cnt_w:
            print(f"  이미 처리된 노드 {cnt_node} (기존 거리 {route[cnt_node]} < {cnt_w}) → 스킵")
            continue
        
        for adjacency_node, adjacency_w in edges[cnt_node]:
            cal_w = route[cnt_node] + adjacency_w
            print(f"  {cnt_node} → {adjacency_node} (간선 비용: {adjacency_w}, 누적 거리: {cal_w})")
            
            if cal_w < route[adjacency_node]:
                print(f"    거리 갱신: {adjacency_node}번 노드의 기존 거리 {route[adjacency_node]} → {cal_w}")
                route[adjacency_node] = cal_w
                heapq.heappush(q, (cal_w, adjacency_node))
                print(f"    힙에 추가: ({cal_w}, {adjacency_node})")
            else:
                print(f"    기존 거리 {route[adjacency_node]}가 더 짧음 → 스킵")
    
    print(f"[다익스트라 종료] {start} → {end} 최단 거리: {route[end]}")
    return route[end]
